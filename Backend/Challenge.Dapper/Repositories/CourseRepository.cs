	...
	public class CourseRepository
	{
		...
		public static async Task<IEnumerable<Course>> GetPaged(int page, int pageSize)
		{
			...
			var result = await connection.QueryAsync<Course>(
				"SELECT * FROM course ORDER BY id OFFSET @skip LIMIT @pageSize",
				new {skip, pageSize});
			await connection.CloseAsync();
			return result;
		}
		...
		public static async Task<Course> Insert(string name)
		{
			...
			await connection.OpenAsync();
			var result =
				await connection.QueryFirstAsync<Course>("INSERT INTO course(name) VALUES (@name)", new {name});
			await connection.CloseAsync();
			return result;
		}
		...
		public static async Task<Course> Update(Course course)
		{
			...
			var result = await connection.QueryFirstAsync<Course>("UPDATE course SET name = @name WHERE id = @ID",
				new {name = course.Name, ID = course.Id});
			await connection.CloseAsync();
			return result;
		}

...
export class TeacherService implements OnInit {
  ...
  getTeachersPaged(page: number, pageSize: number): Observable<PagedData> {
    let headers = new HttpHeaders({ 'Authorization': this.authService.getAuthorizationHeaderValue() });
    return this.http.get<PagedData>(`${environment.apiEndpoint}/Teacher?page=${page}&pageSize=${pageSize}`, { headers: headers });
  }
